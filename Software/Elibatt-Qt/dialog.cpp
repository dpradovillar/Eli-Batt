    ...
    , runButton(new QPushButton(tr("Start")))
{
    ...
    connect(runButton, SIGNAL(clicked()), this, SLOT(transaction()));
    connect(&thread, SIGNAL(response(QByteArray)), this, SLOT(processResponse(QByteArray)));
...
void Dialog::processResponse(const QByteArray &arr)
{
    QString s(arr);
    setControlsEnabled(true);
    trafficLabel->setText(tr("Traffic, transaction #%1").arg(++transactionCount));
    qDebug() << s;
}
