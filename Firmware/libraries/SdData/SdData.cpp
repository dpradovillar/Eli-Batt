...
bool SdWriter::setup(int chipSelectPin, SerialEndpoint *debugEndpoint) {
    ...
    pinMode(10, OUTPUT);
#endif
    d.print("  begining SD with pin:").println((int)m_chip_select_pin);

    if (!SD.begin(m_chip_select_pin)) {
        d.println("Failed initialization of SD card. Check wiring.");
        return false;
    }

    uint16_t filesCount = countFilesInSd();
    d.print("  there are ").print(filesCount).println(" files in the SD card.");
    m_sequence.setStart(filesCount+1);

    /*
    if (!this->open()) {
        d.println("Can't open file!");
        return false;
    }*/
...
bool SdWriter::open() {
    if (m_open) {
        d.println("File already open, trying to close");
        bool could = close();
        if(could) {
            d.println("It did close the file");
        } else {
            d.println("Couldn't close the file, returning");
            return false;
        }
    } else {
        d.println("File wasn't open, skipping closing");
    }

    d.print("Getting next filename from sequence");

    char buffer[13];
    m_sequence.next(buffer);
    buffer[12] = 0;

    d.print("New file open:").println(buffer);

    m_file = SD.open(buffer, FILE_WRITE);
    m_open = (m_file ? true : false);

    d.println(m_open ? "File open!" : "File unopened");
...
bool SdWriter::close() {
    if(m_open) {
        m_file.close();
        m_open = false;
        d.println("Closing file.");
        return true;
    } else {
        d.println("File already closed! nothing to close");
    }
    return false;
}
...
void SdWriter::writeTuple(uint16_t temp, uint16_t current, uint16_t voltage) {
    if (m_open) {
        d.println("Writing to disk");
        m_file.print(","); m_file.print((unsigned int)temp);
        m_file.print(","); m_file.print((unsigned int)current);
        m_file.print(","); m_file.print((unsigned int)voltage);
    } else {
        d.println("Throwing data away because file is not open!");
    }
}
